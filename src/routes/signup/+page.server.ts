...
export const actions = {
    push: async ({ request }: { request: Request }) => {
        ...
        if (
            ...
            return fail(400, { mode: 'error', message: 'Invalid data in the form.' });
        ...
        try {
            ...
            return { mode: 'success', message: 'Account has been created successfully.' };
        } catch (error) {
            console.log(error);
            return fail(400, { mode: 'error', message: 'An unexpected error occurred. Please try again.' });
        }
    }
};
